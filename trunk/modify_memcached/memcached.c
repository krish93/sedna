...
/*
 * Add by Daidong. Handle Socket Stuff.
 */
static int readable_timeout(int fd, int sec){
    fd_set rset;
    struct timeval tv;
...
static int try_talk_buddy(char *buddy_ip) {
    ...
    if ( (nlp_host = gethostbyname(buddy_ip)) == 0){
...
static char* talk_to_buddy(int sd, char *msg) {
    ...
    sf = send(sd, msg, len, 0);
    if (sf == -1){
        printf("Send %s error\n", msg);
        sprintf(buf, "send error");
        return buf;
    }
    if (readable_timeout(sd, 3) == 0){
 ...
 * author: ChenTao
 * Try to use thread to do dirty stuff, do not block memcached process
 * author: Daidong
 */
...
static void *thread_dup(void *args){
    token_t *tokens  = (token_t *) args;
    token_t *ht_num = &tokens[HT_NUM];
    token_t *ip_addr = &tokens[IP_ADDR];
    ...
    if (strcmp(ip, address) == 0){
        printf("dup thread exit same address\n");
        pthread_exit("ok");
    }
    ...
    if (sd == -1){
        printf("dup thread exit connect\n");
        pthread_exit("fail");
    }
    talk_to_buddy(sd, command);
    close(sd);
    printf("dup thread exit\n");
    pthread_exit("ok");
}
...
static void process_duplicate_command(conn *c, token_t *tokens, const size_t ntokens) {
    pthread_t id;
    int ret = pthread_create(&id, NULL, thread_dup, (void *)tokens);
    if ( ret == 0) {
        add_iov(c, "OK\r\n", 4);
    } else {
        add_iov(c, "FAIL\r\n", 6);
...
static int ask_remote_set(int sd, char *key, char *value) {
    ...
    char *command = malloc(2 * 1024 * 1024 * sizeof (char));
    memset(command, 0, 2 * 1024 * 1024 * sizeof (char));
...
static void process_transferht_command(conn *c, token_t *tokens, const size_t ntokens) {
    ...
    printf("trans hashtable num = %d.\n", atoi(ht_num ->value));

    unsigned int hash_power = assoc_get_hashpower(atoi(ht_num ->value));
    
    if (hash_power == 1234567){
        //no such vnode exists here.
        add_iov(c, "fail", 4);
        conn_set_state(c, conn_mwrite);
        c->msgcurr = 0;
        return;
...
static void *thread_dup_ht(void *args){
    token_t * tokens = (token_t *)args;
    ...
    printf("dup hashtable num = %d.\n", atoi(ht_num ->value));

    unsigned int hash_power = assoc_get_hashpower(atoi(ht_num ->value));
 
    if (hash_power == 1234567){
        printf("dup ht exit, hash_power\n");
        pthread_exit("fail");
    }
    ...
    for (i = 0; i < end_count; i++) {
        for (it = hashtable[i]; NULL != it; it = it->next) {
            if (0 == ask_remote_set(sd, ITEM_key(it), ITEM_data(it))) {
                close_remote4set(sd);
                printf("dup ht exit, set\n");
                pthread_exit("fail");
            }
        }
    }
    close_remote4set(sd);
    printf("dup ht thread exit\n");
    pthread_exit("ok");
}
...
static void process_duplicateht_command(conn *c, token_t *tokens, const size_t ntokens) {
    ...
    ret = pthread_create(&id, NULL, thread_dup_ht, (void *)tokens);
